...
void Simulator::Controlled_T(int cont, int targ)
{
    assert((cont >= 0) & (cont < n));
    assert((targ >= 0) & (targ < n));
    assert(cont != targ);

    DdNode *m, *tmp, *term1, *term2;
    DdNode *g, *c;
    int overflow_done = 0;

    m = Cudd_bddAnd(manager, Cudd_bddIthVar(manager, cont), Cudd_bddIthVar(manager, targ));
    Cudd_Ref(m);
    ...
    for (int i = 0; i < w; i++)
    {
        for (int j = 0; j < r; j++)
        {
            copy[i][j] = Cudd_bddAnd(manager, Cudd_ReadOne(manager), All_Bdd[i][j]);
            Cudd_Ref(copy[i][j]);
        }
    }
    ...
    for (int i = 0; i < w - 1; i++)
    {
        for (int j = 0; j < r; j++)
        {
            term1 = Cudd_bddAnd(manager, copy[i][j], Cudd_Not(m));
            Cudd_Ref(term1);
            term2 = Cudd_bddAnd(manager, copy[i + 1][j], m);
            Cudd_Ref(term2);
            Cudd_RecursiveDeref(manager, All_Bdd[i][j]);
            All_Bdd[i][j] = Cudd_bddOr(manager, term1, term2);
            Cudd_Ref(All_Bdd[i][j]);
    ...
    for (int j = 0; j < r; j++)
    {
        term1 = Cudd_bddAnd(manager, copy[w - 1][j], Cudd_Not(m));
        Cudd_Ref(term1);
        term2 = Cudd_bddAnd(manager, Cudd_Not(copy[0][j]), m);
        Cudd_Ref(term2);
        g = Cudd_bddOr(manager, term1, term2);
        Cudd_Ref(g);
        Cudd_RecursiveDeref(manager, term1);
        ...
        if ((j == r - 1) && !overflow_done && overflow2(g, c))
        {
            if (isAlloc)
            {
                r += inc;
        ...
        Cudd_RecursiveDeref(manager, All_Bdd[w - 1][j]);
        All_Bdd[w - 1][j] = Cudd_bddXor(manager, g, c);
        Cudd_Ref(All_Bdd[w - 1][j]);
...
void Simulator::MAJ_Controlled_X(int targ, int c1, int c2, int c3)
{
    assert((targ >= 0) & (targ < n));
    assert((c1 >= 0) & (c1 < n));
    assert((c2 >= 0) & (c2 < n));
    assert((c3 >= 0) & (c3 < n));
    assert(targ != c1 && targ != c2 && targ != c3);
    assert(c1 != c2 && c1 != c3 && c2 != c3);

    DdNode *pair12 = Cudd_bddAnd(manager, Cudd_bddIthVar(manager, c1), Cudd_bddIthVar(manager, c2));
    Cudd_Ref(pair12);
    DdNode *pair23 = Cudd_bddAnd(manager, Cudd_bddIthVar(manager, c2), Cudd_bddIthVar(manager, c3));
    Cudd_Ref(pair23);
    DdNode *pair13 = Cudd_bddAnd(manager, Cudd_bddIthVar(manager, c1), Cudd_bddIthVar(manager, c3));
    Cudd_Ref(pair13);
    DdNode *tmp = Cudd_bddOr(manager, pair12, pair23);
    Cudd_Ref(tmp);
    Cudd_RecursiveDeref(manager, pair12);
    Cudd_RecursiveDeref(manager, pair23);
    DdNode *M = Cudd_bddOr(manager, tmp, pair13);
    Cudd_Ref(M);
    Cudd_RecursiveDeref(manager, tmp);
    Cudd_RecursiveDeref(manager, pair13);

    for (int i = 0; i < w; i++)
    {
        for (int j = 0; j < r; j++)
        {
            DdNode *term1 = Cudd_bddAnd(manager, All_Bdd[i][j], Cudd_Not(M));
            Cudd_Ref(term1);
            tmp = Cudd_Cofactor(manager, All_Bdd[i][j], Cudd_bddIthVar(manager, targ));
            Cudd_Ref(tmp);
            DdNode *term2 = Cudd_bddAnd(manager, tmp, M);
            Cudd_Ref(term2);
            Cudd_RecursiveDeref(manager, tmp);
            tmp = Cudd_bddAnd(manager, term2, Cudd_Not(Cudd_bddIthVar(manager, targ)));
            Cudd_Ref(tmp);
            Cudd_RecursiveDeref(manager, term2);
            term2 = tmp;

            tmp = Cudd_Cofactor(manager, All_Bdd[i][j], Cudd_Not(Cudd_bddIthVar(manager, targ)));
            Cudd_Ref(tmp);
            DdNode *term3 = Cudd_bddAnd(manager, tmp, M);
            Cudd_Ref(term3);
            Cudd_RecursiveDeref(manager, tmp);
            tmp = Cudd_bddAnd(manager, term3, Cudd_bddIthVar(manager, targ));
            Cudd_Ref(tmp);
            Cudd_RecursiveDeref(manager, term3);
            term3 = tmp;
            ...
            All_Bdd[i][j] = Cudd_bddOr(manager, term1, term2);
            Cudd_Ref(All_Bdd[i][j]);
            tmp = Cudd_bddOr(manager, All_Bdd[i][j], term3);
            Cudd_Ref(tmp);
            Cudd_RecursiveDeref(manager, All_Bdd[i][j]);
            All_Bdd[i][j] = tmp;
            Cudd_RecursiveDeref(manager, term1);
