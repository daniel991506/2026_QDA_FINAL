        ...
        {        
            ...
            else
            {
                ...
                else if (inStr == "majx")
                {
                    ...
                    getline(inStr_ss, inStr, ']');
                    targ = stoi(inStr);
                    MAJ_Controlled_X(targ, c1, c2, c3);
                }
                else
                {
                    std::cerr << std::endl
...
void Simulator::sim_qasm(std::string qasm)
{
    ...
    if (has_multi_initial_states())
    {
        ...
        for (std::string label : labels_to_access())
        {
            ...
            {
                print_basis_probabilities(label);
            }
            else if (sim_type == 4)
            {
                print_basis_extremes(label);
            }
            else if (sim_type == 5)
            {
                print_basis_range(min_prob_val, max_prob_val, label);
            }
            else
            {
    ...
    {
        print_basis_probabilities();
    }
    else if (sim_type == 4)
    {
        print_basis_extremes();
    }
    else if (sim_type == 5)
    {
        print_basis_range(min_prob_val, max_prob_val);
    }
    else
    {
        std::cerr << "[Error]: unknown sim_type." << std::endl;
...
void Simulator::execute_gate_line(std::string gate, std::stringstream& inStr_ss)
{
    std::string inStr;
    if (gate == "x")
    {
    ...
    else if (gate == "s")
    {
        getline(inStr_ss, inStr, '[');
        getline(inStr_ss, inStr, ']');
        Phase_shift(2, stoi(inStr));
    }
    else if (gate == "sdg")
    {
        getline(inStr_ss, inStr, '[');
        getline(inStr_ss, inStr, ']');
        Phase_shift_dagger(-2, stoi(inStr));
    }
    else if (gate == "t")
    {
        getline(inStr_ss, inStr, '[');
        getline(inStr_ss, inStr, ']');
        Phase_shift(4, stoi(inStr));
    }
    else if (gate == "tdg")
    {
        getline(inStr_ss, inStr, '[');
        getline(inStr_ss, inStr, ']');
        Phase_shift_dagger(-4, stoi(inStr));
    }
    else if (gate == "rx(pi/2)")
    {
        getline(inStr_ss, inStr, '[');
        getline(inStr_ss, inStr, ']');
        rx_pi_2(stoi(inStr));
    }
    else if (gate == "ry(pi/2)")
    {
        getline(inStr_ss, inStr, '[');
        getline(inStr_ss, inStr, ']');
        ry_pi_2(stoi(inStr));
...
void Simulator::execute_qasm_block(std::string block)
{
    ...
    while (getline(inFile_ss, inStr))
    {
        ...
        if (inStr.find("while") == 0)
        {
            ...
            int bracket_count = (inStr.find("{") != std::string::npos) ? 1 : 0;
            std::string loop_line;
            while (getline(inFile_ss, loop_line))
            {
                if (loop_line.find("{") != std::string::npos)
                    bracket_count++;
                if (loop_line.find("}") != std::string::npos)
                    bracket_count--;
                if (bracket_count == 0)
        ...
        if (inStr == "OPENQASM" || inStr == "include" || inStr == "qreg" || inStr == "creg" || inStr == "}")
            continue;
        if (inStr == "measure")
        {
            ...
            int outcome = measure_and_collapse(qIndex);
            if (cIndex >= live_creg.size())
                live_creg.resize(cIndex + 1, 0);
        ...
        {
            execute_gate_line(inStr, inStr_ss);
        }
    }
}
...
void Simulator::sim_dynamic(std::string qasm)
{
    ...
    n = total_qubits;
    nClbits = total_clbits;
    init_simulator(total_qubits);

    auto run_label = [&](std::string label) {
        final_counts.clear();
        DdNode ***initial_snapshot = copy_current_state();
        for (int s = 0; s < shots; s++)
        {
            restore_from_state_copy(initial_snapshot);
            nClbits = total_clbits;
        ...
        free_state_copy(initial_snapshot);
