...
int main(int argc, char **argv)
{
    ...
    ("type",     po::value<unsigned int>()->default_value(0),    "the simulation type being executed.\n"
                                                                 ...
                                                                 "3: basis probability mode, where probabilities of all computational basis states will be shown.\n"
                                                                 "4: extreme basis probability mode, where only highest and lowest computational basis states are printed.\n"
                                                                 "5: ranged basis probability mode, where only basis states with probability in [min, max] are printed.")
    ...
    ("min",      po::value<long double>()->default_value(0.0L),   "minimum probability for type 5.")
    ("max",      po::value<long double>()->default_value(1.0L),   "maximum probability for type 5.")
    ...
    if (type == 5)
    {
        long double min_val = vm["min"].as<long double>();
        long double max_val = vm["max"].as<long double>();
        if (min_val < 0.0L || max_val < 0.0L || min_val > max_val)
        {
            std::cerr << "[Error]: For type 5, --min and --max must satisfy 0 <= min <= max." << std::endl;
            return -1;
        }
    }
    ...
    assert(shots > 0);
    Simulator simulator(type, shots, seed, r, isReorder, isQuery, isAlloc);
    simulator.min_prob_val = vm["min"].as<long double>();
    simulator.max_prob_val = vm["max"].as<long double>();
