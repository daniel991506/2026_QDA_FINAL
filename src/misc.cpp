...
DdNode ***Simulator::copy_current_state()
{
...
void Simulator::free_state_copy(DdNode ***state)
{
    if (!state)
        return;
    for (int i = 0; i < w; i++)
    {
        for (int j = 0; j < r; j++)
            Cudd_RecursiveDeref(manager, state[i][j]);
        delete[] state[i];
    }
    delete[] state;
}

void Simulator::restore_from_state_copy(DdNode ***state)
{
    if (!state)
        return;
    if (bigBDD)
...
void Simulator::reset_shot(int total_qubits)
{
    if (bigBDD)
    {
        Cudd_RecursiveDeref(manager, bigBDD);
