...
std::pair<long double, long double> Simulator::get_amplitude_value(int *assign)
{
    std::string amplitude = get_amplitude_string(assign);
    if (amplitude == "0")
        return {0.0L, 0.0L};

    size_t plus_pos = amplitude.find(" + ");
    size_t i_pos = amplitude.find("i");
    if (plus_pos == std::string::npos || i_pos == std::string::npos)
    {
        char *end = nullptr;
        long double real = strtold(amplitude.c_str(), &end);
        if (*end == '\0')
            return {real, 0.0L};
        return {0.0L, 0.0L};
    }

    long double real = strtold(amplitude.substr(0, plus_pos).c_str(), nullptr);
    long double imag = strtold(amplitude.substr(plus_pos + 3, i_pos - plus_pos - 3).c_str(), nullptr);
    return {real, imag};
}

void Simulator::print_basis_probabilities(std::string label)
{
    int *assign = new int[n];
    ...
    std::cout << "(basis/prob)" << std::endl;
    for (unsigned long long i = 0; i < nEntries; i++)
    {
        auto amp = get_amplitude_value(assign);
        long double prob = amp.first * amp.first + amp.second * amp.second;
        for (int q = n - 1; q >= 0; q--)
            std::cout << assign[q];
        std::cout << "/" << std::setprecision(12) << prob;
        if (i != nEntries - 1)
            std::cout << " , ";
        full_adder_plus_1(n, assign);
    }
...
void Simulator::print_basis_range(long double minp, long double maxp, std::string label)
{
    int *assign = new int[n];
    unsigned long long nEntries = 1ULL << n;
    for (int i = 0; i < n; i++)
    ...
    if (label != "")
        std::cout << "\"initial_state\": \"" << label << "\"" << std::endl;
    std::cout << "(basis/prob)" << std::endl;
    std::cout << std::setprecision(12) << minp << "<=prob<=" << maxp << ":  ";
    bool first = true;
    for (unsigned long long i = 0; i < nEntries; i++)
    {
        auto amp = get_amplitude_value(assign);
        long double prob = amp.first * amp.first + amp.second * amp.second;
        if (prob + 1e-18L >= minp && prob - 1e-18L <= maxp)
        {
            if (!first)
                std::cout << " , ";
            first = false;
...
void Simulator::print_basis_extremes(std::string label)
{
    int *assign = new int[n];
    unsigned long long nEntries = 1ULL << n;
    for (int i = 0; i < n; i++)
    ...
    long double min_prob = std::numeric_limits<long double>::infinity();
    std::vector<std::string> max_states;
    std::vector<std::string> min_states;
    ...
    for (unsigned long long i = 0; i < nEntries; i++)
    {
        ...
        std::string state;
        for (int q = n - 1; q >= 0; q--)
            state += char('0' + assign[q]);

        if (prob > max_prob + 1e-18L)
        {
            max_prob = prob;
            max_states.clear();
            max_states.push_back(state);
        }
        else if (fabsl(prob - max_prob) <= 1e-18L)
            max_states.push_back(state);

        if (prob < min_prob - 1e-18L)
        {
            min_prob = prob;
            min_states.clear();
            min_states.push_back(state);
        }
        else if (fabsl(prob - min_prob) <= 1e-18L)
            min_states.push_back(state);
    ...
    std::cout << "(basis/prob)" << std::endl;
    std::cout << "highest: ";
    for (size_t i = 0; i < max_states.size(); i++)
    {
        std::cout << max_states[i] << "/" << std::setprecision(12) << max_prob;
        if (i + 1 < max_states.size())
            std::cout << " , ";
    }
    std::cout << std::endl << "lowest: ";
    for (size_t i = 0; i < min_states.size(); i++)
    {
        std::cout << min_states[i] << "/" << std::setprecision(12) << min_prob;
        if (i + 1 < min_states.size())
            std::cout << " , ";
    }
    std::cout << std::endl;
    delete[] assign;
